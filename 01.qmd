...
# Imagine a population with mean = 10 and sd = 2

pop_mean <- 10

pop_sd   <- 2

# Draw many samples of different sizes and compute their means

sizes <- c(20, 50, 200)

means_list <- lapply(sizes, function(n) {

  replicate(1000, mean(rnorm(n, pop_mean, pop_sd)))
...
for (j in seq_along(sizes)) {
  ...
  hist(means_list[[j]], main = paste0("n = ", sizes[j]),

       xlab = "Sample mean", breaks = 20, xlim = c(8.75, 11.75))
...
1. The sample means continue to center around the true mean of 10.
2. The spread of the sample means shrinks. Larger samples produce more stable, less “wiggly” estimates.
...
<!-- visits <- rpois(n, lambda = 2)          # discrete: ER visits last year -->

<!-- bp     <- rnorm(n, mean = 120, sd = 14) # continuous: systolic BP (mmHg) -->

<!-- op <- par(mfrow = c(1,2), mar = c(4,4,2,1)) -->

<!-- barplot(table(visits), xlab = "ER visits (count)", ylab = "Frequency", -->

<!--         main = "Discrete (counts)") -->

<!-- hist(bp, breaks = 20, xlab = "Systolic BP (mmHg)", main = "Continuous (measurement)") -->
...
6. **Quantify uncertainty.**
   Report estimates along with measures of uncertainty, such as confidence intervals and (when appropriate) p-values. Always distinguish **statistical significance** from **practical significance**. These tools will be developed in Chapters 9 and 10.
